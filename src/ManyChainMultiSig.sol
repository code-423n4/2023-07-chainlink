...
/// root is being set.
///
/// Once a root is registered, *anyone* is allowed to furnish proofs of op inclusion in
/// the Merkle tree and execute the corresponding op. The contract enforces that
/// ops are executed in the correct order and with the correct arguments. A notable exception to
/// this is the gas limit of the call, which can be freely determined by the executor. We expect
/// callees to revert if they run out of gas.
///
...
contract ManyChainMultiSig is Ownable2Step {
    ...
    struct Config {
        ...
        // groupQuorums[i] stores the parent group of the i-th signer group. We ensure that the
